...
const BarChart = ({showTable = false}) => {
  ...
  function getMonthFromDate(fecha) {
    const months = [
    ...
    const mesIndex = fechaObj.getMonth(); // El índice del mes (0-11)

    return months[mesIndex];
  }
  ...
  const options = {
    ...
    onClick: (event, elements) => {
      ...
      if (elements && elements.length > 0) {
        ...
        const selectedMonth = data.labels[index]; // Obtener el nombre del mes seleccionado
        // Filtrar los datos de bolideData por el mes seleccionado
        const filteredData = bolideData.filter((item) => {
          return getMonthFromDate(item.date) === selectedMonth;
        });
